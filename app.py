...
def sign_up():
    ...
    doc = {
        ...
        "profile_name": username_receive,                           # 프로필 이름 기본값은 아이디   ---->  사용하지 않음
        "profile_pic": "",                                          # 프로필 사진 파일 이름         ----> 사용하지 않음
        "profile_pic_real": "profile_pics/profile_placeholder.png", # 프로필 사진 기본 이미지       ----> 사용하지 않음 
        "profile_info": ""                                          # 프로필 한 마디                ----> 사용하지 않음 
    }
...
def user(username):
    ...
    # 해당 사용자의 payload 정보가 만료되었거나 또는 payload 정보가 존재하지 않을 경우 home.html로 이동
